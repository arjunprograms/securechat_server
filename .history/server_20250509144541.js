...
// Update the query function to handle no database connection
async function query(text, params) {
  if (!pool) {
    console.error('Database not connected, returning empty result');
    return [];
  }
...
// File upload endpoint - Firebase Storage
// Replace your existing upload endpoint with this simplified version
...
app.post('/upload', (req, res) => {
  ...
  // Create a very simple response as a test
  const simpleResponse = { status: 'success', message: 'Test response' };
  console.log('Sending simple response:', simpleResponse);
  
  // Send JSON response directly with res.json() - simplest possible response
  return res.json(simpleResponse);
});
...
// Add this AFTER all your routes but BEFORE the server.listen call
