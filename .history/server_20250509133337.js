...
admin.initializeApp({
  credential: admin.credential.cert(serviceAccount),
  storageBucket: 'subedi-chat-files.appspot.com'  // ← your bucket name
});

const bucket = admin.storage().bucket();

// --- File upload endpoint (to Firebase Storage) ---
const formidable = require('formidable');
const fs        = require('fs');
...
app.post('/upload', (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    const destFilename = `${Date.now()}_${file.originalFilename}`;
    ...
    // pipe the temporary file into the GCS upload stream
