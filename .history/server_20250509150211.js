...
app.post('/upload', (req, res) => {
  ...
  if (!bucket) {
    ...
    return res.json({ error: 'File storage service is not available' }); // Use res.json instead of res.status().json()
  }
  ...
  form.parse(req, async (err, fields, files) => {
    ...
    try {
      // LOCAL FALLBACK OPTION - Use for initial testing
      // Comment out the Firebase code and uncomment this section if you want to test with local storage
      /*
...
// Add test endpoint to verify JSON responses
app.post('/upload-test', (req, res) => {
  console.log('Test upload endpoint hit');
  res.header('Content-Type', 'application/json');
  res.json({ success: true, message: 'Test upload endpoint working' });
});
