...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    await query('UPDATE users SET status = $1 WHERE username = $2', ['online', username]);
...
app.get('/public-keys', async (req, res) => {
  try {
    const result = await query('SELECT username, public_key FROM users');
    const keys = {};
    result.rows.forEach(r => { if (r.public_key) keys[r.username] = r.public_key; });
    res.json(keys);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
...
app.get('/user-profiles', async (req, res) => {
  try {
    const result = await query('SELECT username, display_name, status FROM users');
    const profiles = {};
...
wss.on('connection', ws => {
    ...
    try {
      ...
      if (data.type === 'auth') {
        ...
        if (user && await bcrypt.compare(data.password, user.password_hash)) {
          ...
          if (data.publicKey) await query('UPDATE users SET public_key = $1 WHERE username = $2', [data.publicKey, currentUser]);
          await query('UPDATE users SET status = $1 WHERE username = $2', ['online', currentUser]);
      ...
      switch (data.type) {
        case 'message': {
          const msg = { type: 'message', id: data.messageId || generateUniqueId(), username: currentUser, content: data.content, encrypted: !!data.encrypted, formatted: !!data.formatted, recipient: data.recipient || 'all', time: new Date().toISOString() };
          await query('INSERT INTO messages(id,sender,recipient,content,is_encrypted,timestamp) VALUES($1,$2,$3,$4,$5,$6)', [msg.id, msg.username, msg.recipient, msg.content, msg.encrypted ? 1 : 0, msg.time]);
        ...
        case 'file': {
          const f = { type: 'file', id: generateUniqueId(), username: currentUser, fileUrl: data.fileUrl, filename: data.filename, fileType: data.fileType, encrypted: !!data.encrypted, recipient: data.recipient || 'all', time: new Date().toISOString() };
          await query('INSERT INTO files(id,sender,recipient,filename,file_url,file_type,is_encrypted,timestamp) VALUES($1,$2,$3,$4,$5,$6,$7,$8)', [f.id, f.username, f.recipient, f.filename, f.fileUrl, f.fileType, f.encrypted ? 1 : 0, f.time]);
        ...
        case 'read_receipt_ack': {
          ...
          if (dest && dest.readyState === WebSocket.OPEN) dest.send(JSON.stringify({ type: 'read_receipt', messageId: data.messageId, reader: currentUser, status: 'read', time: new Date().toISOString() }));
          break;
        }
      }
    } catch (err) {
  ...
  ws.on('close', async () => {
    ...
    await query('UPDATE users SET status = $1 WHERE username = $2', ['offline', currentUser]);
