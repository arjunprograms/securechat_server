...
app.post('/upload', (req, res) => {
  ...
  try {
    // Create the form parser with basic options
    ...
    form.parse(req, (err, fields, files) => {
      ...
      try {
        ...
        // Log success
        console.log(`File uploaded: ${originalName} -> ${fileUrl}`);
        ...
        return res.json({ error: 'File processing failed: ' + fileErr.message });
      }
    });
  } catch (outerErr) {
    console.error('Outer upload error:', outerErr);
    return res.json({ error: 'Upload error: ' + outerErr.message });
  }
});
